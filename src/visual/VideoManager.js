...
export class VideoManager {
    ...
    initVisuals() {
        ...
        // We initially need dummy elements to avoid WebGL warnings
    ...
    addVideos(files, onProgress) {
        Array.from(files).forEach(file => {
            ...
            // Simule progress (real progress events are tricky with blob urls sometimes)
            vObj.status = 'ready'; // Assume blob is fast
    ...
    triggerVideo(videoId) {
        ...
        // Decide Deck
        // If Active is A (mix=0), we load to B and fade to B (mix=1)
        // If Active is B (mix=1), we load to A and fade to A (mix=0)
        // Except if already transitioning?

        let targetDeck = (this.activeDeck === 'A') ? 'B' : 'A';

        // Setup Texture
        const newTex = new THREE.VideoTexture(vObj.element);
        // Optimize: use correct color space
        newTex.colorSpace = THREE.SRGBColorSpace;

        if (targetDeck === 'B') {
            this.deckB.videoId = videoId;
            this.deckB.texture.dispose(); // Dispose old
